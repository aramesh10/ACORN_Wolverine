...
template <typename dist_t>
void readQuerys(int32_t &query_sum,
                int32_t &query_dim,
                dist_t *&querys,
                     ...
                     bool (*predicate)(const acornlib::labeltype))
{
    ...
    for (int i = 0; i < groundtruth_sum; i++) {
        ...
        for (int j = 0; j < groundtruth_dim; j++) {
            ...
            if (label != acornlib::INVALID_LABEL && predicate(label)) {
                groundtruth[i * groundtruth_dim + write_idx] = label;
                write_idx++;
            }
        }
        for (int j = write_idx; j < groundtruth_dim; j++) {
            groundtruth[i * groundtruth_dim + j] = acornlib::INVALID_LABEL;
        }
    }
}

template <typename dist_t>
void creat_index(acornlib::ACORN<dist_t> *&alg_hnsw,
                 string index_prefix,
                 ...
                 int num_threads)
{
    ...
    if (access(index_path.c_str(), F_OK) == 0)
    {
        ...
        alg_hnsw = new acornlib::ACORN<dist_t>(
            space,
            index_path,
            false,
    ...
    else
    {
        ...
        alg_hnsw = new acornlib::ACORN<dist_t>(
            space,
            static_cast<size_t>(max_elements + 1),
...
template <typename dist_t>
pair<float, double> search_index(acornlib::ACORN<dist_t> *alg_hnsw,
                                 int K,
                                 int32_t query_sum,
                                 int32_t query_dim,
                                 dist_t *querys,
                                 ...
                                 bool (*predicate)(const acornlib::labeltype))
{
    ...
    vector<std::priority_queue<std::pair<dist_t, acornlib::labeltype>>> result(query_sum);
    gettimeofday(&search_start_time, NULL);
    ParallelFor(0, query_sum, num_threads, [&](size_t row, size_t threadId)
                { result[row] = alg_hnsw->searchKnn((void *)((char *)querys + row * query_dim * sizeof(dist_t)),
                                                    K,
                                                    predicate); });
    ...
    ParallelFor(0, query_sum, num_threads, [&](size_t row, size_t threadId)
        ...
        while (!result[row].empty()){
            acornlib::labeltype label = result[row].top().second;
...
void creat_deleteList(vector<size_t> &deleteList,
                      int pre_creat_sum,
                      int list_len,
                      default_random_engine &random,
                      uniform_int_distribution<size_t> &dis1)
{
    deleteList.clear();
...
template <typename dist_t>
double psedo_deleteIndex(acornlib::ACORN<dist_t> *alg_hnsw,
...
template <typename dist_t>
double deleteIndex(acornlib::ACORN<dist_t> *alg_hnsw,
                   vector<size_t> deleteList,
                   int delete_model,
                   int num_threads,
                   int newLinkSize)
...
template <typename dist_t>
double addPoint(acornlib::ACORN<dist_t> *alg_hnsw,
                vector<size_t> deleteList,
                int num_threads,
                dist_t *data,
                int32_t dim)
...
template <typename dist_t>
vector<pair<pair<tableint, tableint>, size_t>> getNewEdge(acornlib::ACORN<dist_t> *alg_hnsw1,
                                                          acornlib::ACORN<dist_t> *alg_hnsw2)
{
